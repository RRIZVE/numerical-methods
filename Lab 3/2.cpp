...
void solve()
{
    ...
    long double X,h,first_derivative,second_derivative,u;
    cin >> X;
    h = x[1] - x[0];
    u = (X-x[0])/h;
    long double cal=0,d;
    cal = del_y[1][0] + (2 * u - 1) * (del_y[2][0] / 2) + (3 * u * u - 6 * u + 2) * (del_y[3][0] / 6) + (4 * u * u * u - 18 * u * u + 22 * u - 6) * (del_y[4][0] / 24);
    first_derivative = cal / h;
    cal = del_y[2][0] + (6 * u - 6) * (del_y[3][0] / 6) + (12 * u * u - 36 * u + 22) * (del_y[4][0] / 24);
